...
const HELP_PROMPTS = [
	'Ben kişisel asistanın Rememberry. Bazı günlük temel aktivitelerin nasıl yapıldığını sana hatırlatmak için hazırlandım. Sana destek olmak için buradayım nasıl yardımcı olabilirim?',
	'Ben Rememberry, senin kişisel asistanınım bir aktiviteyi yapmakta sıkıntı yaşadığında benden yardım isteyebilirsin. Şimdi senin için ne yapabilirim?',
];
...
app.intent('actions_intent_PERMISSION', (conv, params, permissionGranted) => {
	if (!permissionGranted) {
		...
		conv.ask(new Suggestions('Sen kimsin?', 'Telefon araması yapmak', 'Karşıdan karşıya geçmek'));
	} else {
		counter = 3;
...
app.intent('Help Intent', (conv) => {
	...
	conv.ask(new Suggestions('Telefon araması yapmak', 'Karşıdan karşıya geçmek'));

});

app.intent('Repeat', (conv) => {
	counter = 3;
	conv.ask("Tekrarlıyorum " + conv.data.lastParen);
...
app.intent('Call Intent - yes', (conv) => {
	...
	conv.ask('Harika ekranın altında ortadaki butona dokunarak tuş kilidini aç. Açtıktan sonra \"devam"\ diyerek devam edebilirsin.-yes');
...
app.intent('Call Intent - no - yes', (conv) => {
	counter = 3;
	conv.data.lastParen = 'Harika ekranın altında ortadaki butona dokunarak tuş kilidini aç.  Açtıktan sonra \"devam"\ diyerek devam edebilirsin.';
	conv.ask('Harika ekranın altında ortadaki butona dokunarak tuş kilidini aç.  Açtıktan sonra \"devam"\ diyerek devam edebilirsin. -no -yes');
	conv.ask(new Suggestions('Devam'));


});

app.intent('Call Intent - no - yes - yes', (conv) => {
	counter = 3;
...
app.intent('Call Intent - no - yes - yes - yes', (conv) => {
	counter = 3;
	...
	conv.ask(new Suggestions('Telefon araması yapmak', 'Karşıdan karşıya geçmek', 'Kapat'));
...
app.intent('Call Intent - no - no - yes', (conv) => {
	...
	conv.ask('Harika ekranın altındaki ortadaki butona dokunarak tuş kilidini aç.  Açtıktan sonra \"devam"\ diyerek devam edebilirsin.-no no-yes');
	conv.ask(new Suggestions('Devam'));
});

app.intent('Call Intent - no - no - yes - yes', (conv) => {
	counter = 3;
	conv.data.lastParen = 'Sonra sol alt köşede üzerinde telefon simgesi olan rehber butonuna dokun. Dokunduktan sonra rehberde aramak istediğiniz ismin üstüne dokunun. Dokunduktan sonra \"devam"\ diyerek devam edebilirsin.';
	conv.ask('Sonra sol alt köşede üzerinde telefon simgesi olan rehber butonuna dokun. Dokunduktan sonra rehberde aramak istediğiniz ismin üstüne dokunun. Dokunduktan sonra \"devam"\ diyerek devam edebilirsin.');
	conv.ask(new Suggestions('Devam'));
...
app.intent('Call Intent - no - no - yes - yes - yes', (conv) => {
	counter = 3;
	conv.data.lastParen = 'Arayacağın kişinin adına dokunduktan sonra arama başlayacaktır. Şimdi ne yapmamı istersin?';
	conv.ask('Arayacağın kişinin adına dokunduktan sonra arama başlayacaktır. Şimdi ne yapmamı istersin?');
	conv.ask(new Suggestions('Telefon araması yapmak', 'Karşıdan karşıya geçmek', 'Kapat'));
...
app.intent('Default Welcome Intent', (conv) => {
	...
	if (!name) {
		...
		conv.ask(new Suggestions('Sen kimsin?', 'Telefon araması yapmak', 'Karşıdan karşıya geçmek', 'Kapat'));
	}
